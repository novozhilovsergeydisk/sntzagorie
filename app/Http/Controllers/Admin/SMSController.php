...
//	define("SMTP_FROM", "api@smsc.ru");     // e-mail адрес отправителя
	...
	CONST SMSC_LOGIN = 'zagorie';
	CONST SMSC_PASSWORD = 'avt811140';
	CONST SMSC_POST = 0;
	CONST SMSC_HTTPS = 0;
	CONST SMSC_CHARSET = 'utf-8';
	CONST SMSC_DEBUG = 0;
	CONST SMTP_FROM = 'admin@sntzagorie.ru';
...
// SMTP версия функции отправки SMS
	...
	{
		return mail("send@send.smsc.ru", "", self::SMSC_LOGIN.":".self::SMSC_PASSWORD.":$id:$time:$translit,$format,$sender:$phones:$message", "From: ".self::SMTP_FROM."\nContent-Type: text/plain; charset=".self::SMSC_CHARSET."\n");
	}
...
//		dump(self::SMSC_LOGIN);
//		return;
		
//		list($sms_id, $sms_cnt, $cost, $balance) = $this->send_sms("79163465407", "Ваш пароль: 999", 1);
		
//		$result = $this->send_sms("79163465407", "Ваш пароль: 999", 1);
...
//		dump(list($sms_id, $sms_cnt, $cost, $balance) = $this->send_sms("79163465407", "Ваш пароль: 999", 1));
//
//		dump($sms_id);
//
//		return;
		...
		$params =[];
		return view('admin.sms', $params)->with('mess', 'index');
	}
	
	public function store() {
		
//		list($sms_id, $sms_cnt, $cost, $balance) = $this->send_sms("79163465407", "Тестовое сообщение: баланс - 1.1 руб.", 1);
		
//		dump('Сообщение от smsc.ru: $sms_id ='.$sms_id.', '.'$sms_cnt = '.$sms_cnt.', '.'$cost = '.$cost.' руб., $balance = '.$balance.' руб.');
		
//		list($status, $time) = $this->get_status(6, "79163465407");
//		dump('$status = '.$status.', $time = '.$time);
//		dump(date('d-m-Y h:i:s', 1518948873));
//		dump($balance = $this->get_balance());
		
//		$balance = $this->get_balance();
//		list($sms_id, $sms_cnt, $cost, $balance) = $this->send_sms("79163465407", "Тестовое сообщение: баланс - ".$balance, 1);
//		dump('Сообщение от smsc.ru: $sms_id ='.$sms_id.', '.'$sms_cnt = '.$sms_cnt.', '.'$cost = '.$cost.' руб., $balance = '.$balance.' руб.');
		
//		list($status, $time) = $this->get_status(6, "79163465407");
//		dump('$status = '.$status.', $time = '.$time);
//		dump(date('d-m-Y h:i:s', 1518948873));
//		dump('$balance = '.$balance = $this->get_balance());
		
		return response()->json(['msg' => $this->get_balance()]);


		
//		$params =[];
//		return view('admin.sms', $params)->with('mess', 'store');
	}
}